...
<3> **방문하지 않은 노드 없음.** 스택 pop (7번 노드)  **스택 pop을 함 으로써 이전 위치로 돌아간다.**

![dfs10](images/dfs10.png)

<0> (**현재 스택의 최상단 요소 조회** --> 2)

<1> 현재 노드(2)에 연결된 노드 (1, 7) 조회

<2> 이중에서 방문하지 않은 노드()만 남김

<3> **방문하지 않은 노드 없음.** 스택 pop (2번 노드)  **스택 pop을 함 으로써 이전 위치로 돌아간다.**

![dfs11](images/dfs11.png)

<0> (**현재 스택의 최상단 요소 조회** --> 1)

<1> 현재 노드(1)에 연결된 노드 (2, 3) 조회

<2> 이중에서 방문하지 않은 노드(3)만 남김

<3> 방문하지 않은 노드가 있으므로 스택 pop 하지 않음

<4> 가장 첫번째 노드(3) 스택에 삽입

<5> 삽입된 노드(3) 방문 처리

![dfs12](images/dfs12.png)

<0> (**현재 스택의 최상단 요소 조회** --> 3)

<1> 현재 노드(3)에 연결된 노드 (4, 5) 조회

<2> 이중에서 방문하지 않은 노드(4, 5)만 남김

<3> 방문하지 않은 노드가 있으므로 스택 pop 하지 않음

<4> 가장 첫번째 노드(4) 스택에 삽입

<5> 삽입된 노드(4) 방문 처리

![dfs13](images/dfs13.png)

<0> (**현재 스택의 최상단 요소 조회** --> 4)

<1> 현재 노드(4)에 연결된 노드 (3) 조회

<2> 이중에서 방문하지 않은 노드()만 남김

<3> **방문하지 않은 노드 없음.** 스택 pop (4번 노드)  **스택 pop을 함 으로써 이전 위치로 돌아간다.**

![dfs14](images/dfs14.png)

<0> (**현재 스택의 최상단 요소 조회** --> 3)

<1> 현재 노드(3)에 연결된 노드 (4, 5) 조회

<2> 이중에서 방문하지 않은 노드(5)만 남김

<3> 방문하지 않은 노드가 있으므로 스택 pop 하지 않음

<4> 가장 첫번째 노드(5) 스택에 삽입

<5> 삽입된 노드(5) 방문 처리

![dfs15](images/dfs15.png)

<0> (**현재 스택의 최상단 요소 조회** --> 5)

<1> 현재 노드(5)에 연결된 노드 (3) 조회

<2> 이중에서 방문하지 않은 노드()만 남김

<3> **방문하지 않은 노드 없음.** 스택 pop (5번 노드)  **스택 pop을 함 으로써 이전 위치로 돌아간다.**

![dfs16](images/dfs16.png)
...
<3> **방문하지 않은 노드 없음.** 스택 pop (3번 노드)  **스택 pop을 함 으로써 이전 위치로 돌아간다.**

![dfs17](images/dfs17.png)
...
<3> **방문하지 않은 노드 없음.** 스택 pop (1번 노드)  **스택 pop을 함 으로써 이전 위치로 돌아간다.**


![dfs18](images/dfs17.png)
